<script setup lang="ts">
  ...
  const {
    ...
    onBeforeLeave,
    onLeave,
  } = useHeightTransition({ duration: 300, marginTop: 0, animateOpacity: false, });

  function toggleOpen() {
    if (isBodyVisible.value || isOpen.value) {
  ...
  const sortIcon = computed(() => {
    if (eventsSort.value === 'asc') return 'fluent:arrow-sort-up-16-regular';
    if (eventsSort.value === 'desc') return 'fluent:arrow-sort-down-16-regular';
    return 'fluent:arrow-sort-16-regular';
  });

  watch([isOpen, eventsPending], async ([open, pending]) => {
    if (!open) {
      isBodyVisible.value = false;
      return;
    };
...
<template>
  <li
    class="
      flex flex-col
      bg-main border-1 border-solid border-fifth rounded-md
      transition-shadow transition-300 ease-in-out
  ...
  >
    ...
    >
      <div v-if="isBodyVisible" class=" overflow-hidden">
        <div 
        ...
        >
          <div class="flex items-center justify-between gap-2.5 px-4.5">
            ...
            >
              <p>
                Events: <span>{{ eventsTotal }}</span>
              </p>
            </div>
  
            <UiButton style-type="cancel" @click.stop="toggleEventsSort">
              <Icon 
                :name="sortIcon"
