...
async def invalidate_event_lists(redis: Redis, user_id: int | None = None) -> None:
  ...
  if user_id is not None:
    ...
    await cache_delete_pattern(redis, f"events:by-user={user_id}:*")


async def invalidate_event_participants(redis: Redis, event_id: UUID) -> None:
  await cache_delete_pattern(redis, f"event:{event_id}:participants:*")


async def invalidate_event_completely(redis: Redis, event_id: UUID, user_id: int) -> None:
